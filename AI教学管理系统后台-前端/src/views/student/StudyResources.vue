<template>
  <div class="study-resources">
    <div class="text-box">
      <p>AI的学习成果评价</p>
      <div v-if="!studyResults.length" class="empty-icon">
        <!-- 空图标的SVG -->
        <svg width="100" height="100" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 0C5.373 0 0 5.373 0 12c0 6.627 5.373 12 12 12s12-5.373 12-12c0-6.627-5.373-12-12-12zm0 22C6.486 22 2 17.514 2 12S6.486 2 12 2s10 4.486 10 10-4.486 10-10 10zm-1-9h2v2h-2zm0-10h2v8h-2z"/>
        </svg>
      </div>
      <div v-else class="scroll-box">
        <ul>
          <li v-for="(result, index) in studyResults" :key="index">{{ result }}</li>
        </ul>
      </div>
    </div>
    ...
    <Popup v-if="showPopupFlag" :type="popupType" @close="showPopupFlag = false" />
  </div>
</template>
...
export default {
  ...
  async fetchStudyResults() {
    try {
        ...
        // 发起请求获取学习成果评价
        const response = await axios.get(`http://127.0.0.1:9327/api/student/getStudyResults`, {
          params: { username: username }
        });
        console.log('API response:', response);
        if (response.data.code === 200) {
          // 将学习成果评价按分号拆分为数组
          this.studyResults = response.data.data.split(';');
        } else {
          throw new Error(response.data.msg);
        }
      } catch (error) {
        console.error('获取学习成果评价失败:', error);
      }
    },
  },
  mounted() {
    this.fetchStudyResults();
  },
};
</script>
